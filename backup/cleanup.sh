...
if [ -n "${RCLONE_REMOTE}" ] && [ -n "${RCLONE_PATH}" ]; then
    ...
    # Backup papkasida .sql.gz fayllari mavjudligini tekshirish
    if find "${BACKUP_DIR}" -maxdepth 1 -type f -name "*.sql.gz" -print -quit 2>/dev/null | grep -q .; then
        
        echo "☁️ Qoldiq Fayllarni Yuklash boshlandi."
        
        # Har bir topilgan faylni qayta yuklashga harakat qilish
        find "${BACKUP_DIR}" -maxdepth 1 -type f -name "*.sql.gz" -print0 | while IFS= read -r -d $'\0' FILE_TO_UPLOAD; do
